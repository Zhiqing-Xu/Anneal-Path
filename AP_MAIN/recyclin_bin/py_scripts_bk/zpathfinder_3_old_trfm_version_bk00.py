...
class PathFinder: 
    def __init__(self, pruning_method=None, ignore_chirality=True, outstream=sys.stderr, rxn_diameter=2, rxn_score_lb=0):
        self.ignore_chirality = ignore_chirality
        self.pruning_method = pruning_method
        self.outstream = outstream
        self.reactor = Reactor(rxn_diameter=rxn_diameter, rxn_score_lb=rxn_score_lb)
    def popsicle(self):
        popsicle=["         _ _","       ,'/ //\\","      /// // /)","     /// // //|","    /// // ///","   /// // ///","  (`: // ///","   `;`: ///","   / /:`:/","  / /  `'"," / /","(_/"]
...
# $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ #
    ...
    # ==================================================================================== #
    def MNA_select(self, list_tb_elmnt, list_tb_cmp, depth, num_cmpds=10): # Compare "list to be eliminated" with "list to be compared" and return top "num cmpds" compounds
        # Inputs: a list of hashes, a list of hashes, num compounds to return
        # Output: a list of hashes
        # 1. Convert COMPOUNDS in "list to be eliminated" to "fp1" and obtain maximum taofactors for all compounds
        taofactors_list=[] #
        for hash_a in list_tb_elmnt:
            taofactor=[]
            for hash_b in list_tb_cmp:
                taofactor.append(self.similarity_score(hash_a, hash_b, depth))
            taofactors_list.append((hash_a,max(taofactor)))
        # 2. Sort the taofactors for all compounds and return top ranked compounds
        taofactors_dict={}
        ...
        num_cmpds= num_cmpds if num_cmpds<len(sorted_taofactors_list) else len(sorted_taofactors_list)
        for j in range(num_cmpds):
            if sorted_taofactors_list[j][1] != 0:
                top_ranked_reactants.append(sorted_taofactors_list[j][0])
            else:
                break
        return top_ranked_reactants
    # ==================================================================================== #
            ...
            for i in range(prod_level): # i helps get different length of pathways, pathway_length=i+1+subs_level
                ...
                for j in range(subs_level):
                    ...
                    for one_icmplt_pathway in temp_pathways_list: # n means processing (doing substitution to) the n^th pathway in the pathway list
                        ...
                        ##
                        # still has problem here, have chance to erase good pathways, so currently not employed!!!!!
                        ...
                        if reactantstobeexpanded!=[]:
                            ...
                            for one_rctt_tb_expdd in reactantstobeexpanded: #1.2.2.2
                                ...
                                for one_rxn in reactions_list: #1.2.2.2.1
                                    #print (tuple(set(reactions_list[r][1])) not in reactants_found_already)
                                    if (one_rctt_tb_expdd in one_rxn[0]) and \
                                        iftuplestrinlist(one_rxn[1],fwd_smiles_list_level_list[l-1]) and \
                                        (tuple(set(one_rxn[1])) not in reactants_found_already):
                                        next_level_reactions[reactantstobeexpanded.index(one_rctt_tb_expdd)].append((l,one_rxn[0],one_rxn[1],one_rxn[2]))
                            next_level_reactions_set=cart_prod(next_level_reactions)
...
# $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ #
# $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ #
    def expand_reaction_tree(self,fwd_smiles_set,fwd_rxn_set,fwd_smiles_set_level_list,trfm_rxn_dict,trfm_rule_dict,trfm_odict):
        ...
        print "len(new_rxn_list): ", len(new_rxn_list)
        print "len(set(new_rxn_list)): ", len(set(new_rxn_list))
        #####----------Step 0-1 Update trfm_rxn_dict & trfm_rule_dict.  Modify new_rxn_list. (old version)
        # use the trfm_dict needs to comment out part 0-2
        '''
        modified_new_rxn_list=[]
        for one_rxn in new_rxn_list:
            print count_x
            count_x+=1
            one_trfm=(one_rxn[0],one_rxn[1])
            if trfm_rxn_dict.values().count(one_trfm)==0:
                new_trfm_id=str(len(trfm_rxn_dict)+1)
                trfm_rxn_dict[new_trfm_id]=one_trfm
                trfm_rule_dict[new_trfm_id]=[one_rxn[2],]
                modified_new_rxn_list.append((one_rxn[0],one_rxn[1],new_trfm_id))
            else:

                #trfm_id_index=trfm_rxn_dict.values().index(one_trfm)
                #trfm_id=trfm_rxn_dict.keys()[trfm_id_index]

                trfm_id=trfm_rxn_dict.keys()[trfm_rxn_dict.values().index(one_trfm)]
                trfm_rule_dict[trfm_id].append(one_rxn[2])
                if (one_rxn[0],one_rxn[1],trfm_id) not in modified_new_rxn_list:
                    modified_new_rxn_list.append((one_rxn[0],one_rxn[1],trfm_id))
        new_rxn_list=modified_new_rxn_list
        ...
        # use the trfm_dict needs to comment out step 0-2
        #####----------trfm_odict
        #####----------trfm_odict
        #####----------trfm_odict
        ...
        for one_rxn in new_rxn_list:
            ...
            one_trfm_ezm_list=trfm_odict.setdefault(one_trfm,[]).append(one_rxn[2])
            trfm_odict[one_trfm]=one_trfm_ezm_list
            trfm_id=trfm_odict.keys().index(one_trfm)
        ...
        for one_rxn in new_rxn_list:
            one_trfm=(one_rxn[0],one_rxn[1])
            updated_enzymes_str=trfm_rxn_dict.setdefault(one_trfm,'')+' '+one_rxn[2]
            trfm_rxn_dict[one_trfm]=updated_enzymes_str
            one_modified_rxn=(one_rxn[0],one_rxn[1],updated_enzymes_str)
            modified_new_rxn_set.add(one_modified_rxn)
        ...
        if subs_level==(max_levels_global+1)/2:
        ...
        # 0. Pre-screening: Remove large compounds (have more C or over 2 more O atoms than the target compound)
            ...
            if subs_level==0:
                ...
                if P0_global==[1,1,1]:
                    ...
                    num_screening_global=P0_global
                ...
                if P0_global==[2,2,2]:
                    for one_smiles in screened_smiles_list:
                        if one_smiles not in fwd_smiles_set:
                            ...
                            fwd_smiles_set.add(one_smiles)
                    print "Accepted All:"
                    print "len(screened_smiles_list): ", len(screened_smiles_list)
                else:
                    # 1. Accept top similarity improvement
                    top_similarity_improvement=probability_select(fwd_smiles_set_level_list,screened_smiles_list,new_rxn_list,target_compound,num_screening_global[0])
...
# $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ #
# $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ #   
    def bwd_expand_reaction_tree(self,bwd_pathways_list,cmplt_pathways_list,bwd_rxn_list,bwd_level_dict,trfm_rxn_dict,trfm_rule_dict,trfm_odict):
        ...
        #####----------Step 0-2 Update trfm_rxn_dict & trfm_rule_dict.  Modify bwd_new_rxn_list. (old version)
        '''
        modified_bwd_new_rxn_list=[]
        for one_rxn in bwd_new_rxn_list:
            ...
            if trfm_rxn_dict.values().count(one_trfm)==0:
                ...
                trfm_id=trfm_rxn_dict.keys()[trfm_id_index]
                trfm_rule_dict[trfm_id].append(one_rxn[2])
            if (one_rxn[1],one_rxn[0],trfm_id) not in modified_bwd_new_rxn_list:
                modified_bwd_new_rxn_list.append((one_rxn[0],one_rxn[1],trfm_id))
        bwd_new_rxn_list=modified_bwd_new_rxn_list
        '''
        ...
        for one_rxn in bwd_new_rxn_list:
            ...
            updated_enzymes_str=trfm_rxn_dict.setdefault(one_trfm,'')+' '+one_rxn[2]
            trfm_rxn_dict[one_trfm]=updated_enzymes_str
            modified_bwd_new_rxn_set.add((one_rxn[0],one_rxn[1],updated_enzymes_str))
        bwd_new_rxn_list=list(modified_bwd_new_rxn_set)
        '''
        #####----------
        ...
        if prod_level==0:
            ...
            if P0_global==[2,2,2]:
                ...
                # 1. Accept top similarity improvement
                top_similarity_improvement=bwd_probability_select(screened_smiles_list,bwd_new_rxn_list,subs_smiles,num_screening_global[0],bwd_level_dict)
                print "Similarity improvement: "
                Similarity_improvement=[]
                for one_smiles in top_similarity_improvement:
                    bwd_current_accepted_smiles_set.add(one_smiles)
        ...
        if bwd_pathways_list[0][0][0][0]==0:
            ...
            print"tb_expanded_list", len(tb_expanded_list)
            ...
            for n in range(len(tb_expanded_list)):
                ...
                if len(reactantstobeexpanded)==0:
                    print "completed pathway found in incomplete pathway list1"
                    continue
        ...
        for z in range(prod_level):
            print "prod_level", z
            cmpd_level=z+1 # cmpd_level is the level of the compounds are to be substituted.
    ...
    def pathway_searching(self, starting_cmpds_smiles_list, target_cmpd_smiles_list, max_levels, max_value):
        ...
        while (subs_level + prod_level < max_levels):
            if subs_level<=prod_level:
                ...
                self.expand_reaction_tree(fwd_smiles_set,fwd_rxn_set,fwd_smiles_set_level_list,trfm_rxn_dict,trfm_rule_dict,trfm_odict)
                ...
                target_pathways_list=self.bwd_expand_reaction_tree(target_pathways_list,complete_pathways_list,backward_reactions_list,backward_level_dict,trfm_rxn_dict,trfm_rule_dict,trfm_odict)
                searching_level += 1
                prod_level += 1
        #@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@#
...
# $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ #
# $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ #
    def solve_pathway(self, original_subs_string, original_prod_string, html_writer, pathway_name, pathway_prefix, max_levels, ifsmiles, XI, T0, P0, fp_type, max_value, bin_adj):
        ...
        if ifsmiles == False:
        # Inputs are SMARTS (need further editing)
            pass
        else:
        # Inputs are SMILES
        ...
        for a in range(len(result_pathways)):
            for b in range(len(result_pathways[a])):
                text_file.write(str(result_pathways[a][b]))
                text_file.write("\n")
            text_file.write("\n")
        text_file.close()
        ...
        text_file = open("../results/" + pathway_name + "/search_log.txt", "w")
        ...
        try:
            with connection.cursor() as cursor:
                sql_query = "INSERT INTO `search_result` (pwy_name,sim_mtrc,max_len,ini_temp,cool_coef,dsim_sel,simu_sel,simi_sel,\
                                subs_sms,prod_sms,max_C_subs,max_C_prod,max_O_prod,max_O_subs,dsim_adj,simu_adj,simi_adj,rxn_time,pwy_time,pwy_num\
                ...
                val_tb_inserted=(pathway_name,str(fp_type_global),max_levels,T0_global,XI_global,P0_global[0],P0_global[1],P0_global[2],\
                                    ...
                                    bin_adj[0],bin_adj[1],bin_adj[2],time1,time2,len(result_pathways))
