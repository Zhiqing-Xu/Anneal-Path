...
class Get_Unique_SMILES:
    ...
    #--------------------------------------------------#
    def UniSS(self, smiles_x): # old_name: UniS()
    ...
    #--------------------------------------------------#
    def UniS(self, smiles_x): # old_name: CanS()
        # SMILES -> R-mol -> SMILES
        SMILES_bool = True
        mol_x=Chem.MolFromSmiles(smiles_x) # MolFromSmiles will NOT return ERROR, no matter what string is input.
        try:
...
def unique_canonical_smiles_AP(smiles_x, isomericSmiles = True, kekuleSmiles = False, canonical = True):
    # SMILES -> R-mol -> SMARTS -> R-mol -> SMILES
    GetUnqSmi = Get_Unique_SMILES(isomericSmiles = isomericSmiles, kekuleSmiles = kekuleSmiles, canonical = canonical)
    return GetUnqSmi.UniSS(smiles_x)[0]
...
#====================================================================================================#
def SmilesToMol_ZX(smiles_x, bad_ss_dict = {}, 
