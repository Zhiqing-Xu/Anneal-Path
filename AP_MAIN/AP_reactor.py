...
#######################################################################################################################################
#######################################################################################################################################
    def pattern_matching_AP(self, cmpd_smiles, rxn_rule_subgraph):
        #####----------return boolean variable (if the compound match the subgraph)
        molecule = MolFromSmiles_AP(cmpd_smiles, bad_ss_dict)
        try:
    ...
    def generate_new_reactions(self, reactants_smiles_list, reaction_id):
        ...
        for one_reactant_smiles in reactants_smiles_list:
            r_fmt_reactants_list.append(MolFromSmiles_AP(one_reactant_smiles, bad_ss_dict))
        ...
        for r_fmt_one_set_products in r_fmt_products_lists:
            ...
            for r_fmt_one_product in r_fmt_one_set_products:
                one_set_products.append(MolToSmiles_AP(r_fmt_one_product, bad_ss_dict))
                ...
                for one_cmpd_smiles in fwd_smiles_set_level_list[subs_level]:
                    ...
                    for one_products_list in products_lists:
                        if one_products_list not in products_list_duplicates:
                            ...
                            for one_smiles in unique_canonical_smiles_list_AP(one_products_list):
            ...
            #####----------Step 1-3 Type Two Enzyme, reaction coef>1 OR two reactants:
                ...
                for i in range(numreactants):
                    for one_cmpd_smiles in fwd_smiles_set.union(set(bkgd_cmpd_list)):
                        if one_cmpd_smiles.find("CoA")!=-1:
                            continue # CoA cannot be processed.
                        if self.pattern_matching_AP(one_cmpd_smiles, reactant_substructure_list[i]):
...
#######################################################################################################################################
#######################################################################################################################################
    def bwd_apply_enzymes_AP(self,cmpds_need_expand,prod_level):
        print ("Reaction Simulation ... ")
...
#######################################################################################################################################
    def test_z(self):
        ...
        #new_rct_list= self.bwd_apply_enzymes_AP(fwd_smiles_set,subs_level)
        print (new_rct_list)
...
#######################################################################################################################################
    ...
    def test_w(self):
        ...
        print ("start!")
        print (MolToSmiles_AP(r_fmt_cmpd_x, bad_ss_dict))
        print (bad_ss_dict)
        print ("finished!")
    ...
    def test_ww(self):
        ...
        print (AP_reactor.pattern_matching_AP("CCO","[CH,CH2,CH3:2][C:4][OH:1]"))
        print (AP_reactor.pattern_matching_AP("O","[O:1]-[C:2]"))
    ...
    def test_y(self):
        ...
        for rxn_id in self.selected_rxn_id_list:
            if self.selected_rxn_dict[rxn_id][0].split(">>")[1].count(".")==0:
                if self.pattern_matching_AP(a, self.selected_rxn_dict[rxn_id][0].split(">>")[1]):
            ...
            if self.selected_rxn_dict[rxn_id][0].split(">>")[0].count(".")==0:
                if self.pattern_matching_AP(a, self.selected_rxn_dict[rxn_id][0].split(">>")[0]):
                    print (self.selected_rxn_dict[rxn_id][5])
...
if __name__ == '__main__':
    ...
    print (AP_reactor.pattern_matching_AP("CC(=O)O","[CH3:1][C:2](=[O:3])[OH:4]"))
    print (AP_reactor.pattern_matching_AP("CC(=O)OP(O)(O)=O","[C:2][O:6][P:7]([OH:8])([OH:9])=[O:10]"))
